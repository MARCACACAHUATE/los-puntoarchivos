...
class NoticiasSpider(scrapy.Spider):
    ...
    def parse_categorias(self, response: TextResponse):
        ...
        for noticia in noticias:
            yield scrapy.Request(url = "https://www.elfinanciero.com.mx" + noticia, callback = self.parse_noticias)


    def parse_noticias(self, response: TextResponse):
        cuerpo = ""
        xpath_noticia = self.xpath["noticias"]
        titulo = response.xpath(xpath_noticia["titulo"]).get()
        cuerpo_no_parsed = response.xpath(xpath_noticia["cuerpo"]).getall()
        fecha = response.xpath(xpath_noticia["fecha"]).get()

        for parrafo in cuerpo_no_parsed:
